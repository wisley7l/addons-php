  ...
  if ($result = mysqli_query(  $conn, $query )) {
    ...
    while ($row = mysqli_fetch_assoc($result)) {
      $item = get_app_theme($row['id'], $row['partner_id'], $row['title'],
        treatImages($row['thumbnail']), treatNumber($row['value_plan_basic']),
        $row['p_id'], treatImages($row['path_image']), $dictionary, 1);
...
function search_all_themes($limit)
{
  ...
  if ($result = mysqli_query(  $conn, $query )) {
    ...
    while ($row = mysqli_fetch_assoc($result)) {
      $item = get_app_theme($row['id'], $row['partner_id'], $row['title'],
        treatImages($row['thumbnail']), treatNumber($row['value_license_basic']),
        $row['p_id'], treatImages($row['path_image']), $dictionary, 0);
...
function search_partner_id($partner)
{
  ...
  $query =  "SELECT p.id, p.member_since, p.path_image, p.profile_json
    ...
    if ($result = mysqli_query(  $conn, $query )) {
      // fetch associative array
      while ($row = mysqli_fetch_assoc($result)) {
        $partner = getInfoUser($row['id'],$row['member_since'],treatImages($row['path_image']),
  ...
  if ($result = mysqli_query(  $conn, $query )) {
    ...
    while ($row = mysqli_fetch_assoc($result)) {
      $item = array(
        ...
        'title'=> $row['title'],
        'slug'=> $row['slug'],
        'thumbnail'=> treatImages($row['thumbnail']),
...
function getImagesApp($id)
{
  ...
  if ($result = mysqli_query(  $conn, $query )) {
    ...
    while ($row = mysqli_fetch_assoc($result)) {
      $item = array(
      'id' => $row['id'],
      'path_img' => treatImages($row['path_image'])
      );
      // var_dump($item);
...
function treatImages($value)
{
  ...
  return '/fig' . substr($value, $delete);
